    ...
    <div className="space-y-6">
            ...
            {customers.map((c) => (
              <tr key={c.id} className="border-b last:border-0">
                ...
                <td className="p-3 text-right">
                  <Link href={`/admin/${companySlug}/clientes/${c.id}`} className="text-primary hover:underline">Ver</Link>
                </td>
              </tr>
            ))}
            {customers.length === 0 && (
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex gap-2">
          {page > 1 && <Link href={`/admin/${companySlug}/clientes?pagina=${page - 1}`} className="text-primary hover:underline">Anterior</Link>}
          {page * perPage < total && <Link href={`/admin/${companySlug}/clientes?pagina=${page + 1}`} className="text-primary hover:underline">Próxima</Link>}
        </div>
      </div>
    </div>
  );
