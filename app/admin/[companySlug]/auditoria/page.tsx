    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex gap-2">
          {page > 1 && <Link href={`/admin/${companySlug}/auditoria?pagina=${page - 1}`} className="text-primary hover:underline">Anterior</Link>}
          {page * perPage < total && <Link href={`/admin/${companySlug}/auditoria?pagina=${page + 1}`} className="text-primary hover:underline">Próxima</Link>}
        </div>
      </div>
    </div>
  );
